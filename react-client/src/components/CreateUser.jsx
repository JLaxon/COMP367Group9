...
function CreateUser(props) {
  ...
  //
  const [user, setUser] = useState({ _id: '', username: '', password: '', firstName: '', lastName: '', address: '', city: '',
    phoneNumber: '', email: '', });
  ...
  const saveUser = (e) => {
    setShowLoading(true);
    e.preventDefault();
    const data = { username: user.username, password: user.password, firstName: user.firstName, lastName: user.lastName, 
      address: user.address, city: user.city, phoneNumber: user.phoneNumber, email: user.email, };
      //use promises
  ...
  const onChange = (e) => {
    e.persist();
    setUser({...yser, [e.target.name]: e.target.value});
  }
